...
# Function to calculate bounding box parameters
def calculate_bounding_box(cube):
    # Find rectangle coordinates
    rectangle_coordinates = find_rectangle_coordinates(cube)

    # Find rectangle center
    center_x, center_y = find_rectangle_center(rectangle_coordinates)

    # Find rectangle directions
    x_direction, y_direction = find_rectangle_directions(rectangle_coordinates)

    # Calculate extent
    z_length, middle_point = min_max_z_coordinates(np.array(cube))
    
    center = (center_x, center_y, middle_point)
    
    extent = [np.linalg.norm(x_direction), np.linalg.norm(y_direction), z_length]
    
    # Rotation matrix
    rot_mat = np.column_stack((x_direction, y_direction, [0, 0, 1]))
...
def centroid_and_box(points,cluster_labels,num_clusters):
    ...
    for cluster_id in range(num_clusters):
        ...
        centroid,extent,rot_mat=calculate_bounding_box(object_points)
