...
def follow_the_path(bg,path,size_x,size_y,spacing):
    ...
    ajust_angle=0
    trajectory=[]
    checkpoints= m.create_checkpoints_with_variable_spacing(path,spacing_away=30,spacing_near=8,threshold=1)
    ...
    while counter<200:
      ...
      final_array=m.merge_arrays(cube, path,bg,Parts,checkpoints)
