...
def next_point_new(bbox,old_teta_e,teta_e,delta_t,v_max,beta):
  rot_mat=bbox.get_rotation_matrix()
  center=bbox.get_center()
  """
  Given the ajustment angle and the velocity that the car moves returns the (x,y,z) of the exact position in the next time step. 
  ...
  vector_p=np.dot(rot_mat, np.array([1,0,0])) [:2]
  #print("vector p", vector_p)
  old_teta_ajust=ajust_angle(beta,old_teta_e,delta_t)
  adj= np.radians(old_teta_ajust)
  adjustment_matrix = np.array([[np.cos(adj), -np.sin(adj)],
                           [np.sin(adj), np.cos(adj)]])
  #Apply the value of this rotation to the x_axis of the boinding box to understand the moving direction.
  pointing_direction = np.dot(adjustment_matrix, vector_p)
...
#Having the bbox and the trajectory is possible to calculate the next position
    ...
    #print("teta_e", teta_e)
    position,calc_angle=next_point_new(bbox,old_teta_e,teta_e,delta_t,v_max,beta)
    return (position, calc_angle)
...
def follow_the_path(bg,path,size_x,size_y,spacing):
    ...
    while counter<200:
      ...
      #Deteção de objectos
      noise=m.create_random_points(20,(0,28),(0,16))
      map=m.merge_arrays(map,noise)
      ...
      for object in tracker:
        ...
        if extent[0]>0.3:
          ...
          if len(trajectory)<=1:
            ...
            #print("OUTRAS VEZES")
            position,calc_angle=other_times(object,teta_e,checkpoints,delta_t,v_max,beta)
      ...
      if encontrou==False and mudou_o_caminho==True:
        ...
        print(find_point_index(info_old[1][0],path))
        if find_point_index(next_check,path)<=find_point_index(info_old[1][0],path):
          print("Path should be corrected")
          path=reupdate_path(info_old[1],path,spacing)
          mudou_o_caminho=False
          checkpoints= m.create_checkpoints_with_variable_spacing(path,spacing_away=larger,spacing_near=smaller,threshold=1,z_offset=1)
      ...
      final=m.merge_arrays(final,noise)
