...
"""def distance_along_vector(points, vector):
    ...
    # Find the minimum and maximum coordinates along the direction
    min_coordinate = np.min(np.dot(points, vector))
    max_coordinate = np.max(np.dot(points, vector))

    # Calculate the distance between the minimum and maximum coordinates
    distance = np.abs(max_coordinate - min_coordinate)
...
def bbox_implementation(cube):
    ...
    center = (mean_vector[0], mean_vector[1], middle_point)
    extent = [projection_x, projection_y, z_length]
    print("EXTENT",extent)
...
def centroid_and_box(points,cluster_labels,num_clusters):
    ...
    for cluster_id in range(num_clusters):
        ...
        if bbox_implementation(object_points) is not None:
